...
class ActorCriticNetwork(nn.Module):
    def __init__(self, lr, input_dims, n_actions, fc1_dims=256, fc2_dims=256):
        ...
        self.scheduler = T.optim.lr_scheduler.StepLR(self.optimizer, step_size=100, gamma=0.1)
...
class Agent():
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_actions, 
        ...
        self.actor_critic = ActorCriticNetwork(lr, input_dims, n_actions, 
                                               fc1_dims, fc2_dims)
        #self.actor_critic.load_state_dict(T.load("weights_only.pth"))
        #self.actor_critic.eval()
        self.log_prob = None

    def choose_action(self, observation, action_chosen = None):
        ...
        if action_chosen is not None:
            action = T.tensor([action_chosen])
            action = action.to(self.actor_critic.device)
        else:
    ...
    def save(self):
        T.save(self.actor_critic.state_dict(), 'weights_only.pth')
